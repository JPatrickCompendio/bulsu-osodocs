...
async function handleChairmanDashboard(url: URL) {
  const supabase = getAdminClient();
  const role = url.searchParams.get('role') || 'chairman';
  const userId = url.searchParams.get('userId');

  const { data: staffUser } = userId
  ...
  const audiences =
    role === 'vice-chairman' ? ['oso-staff', 'vice-chairman'] : ['oso-staff', 'chairman'];

  const announcements = await fetchActiveAnnouncements(supabase, role, '', 3);

  let pendingSubmissions: Array<Record<string, unknown>> | null = null;
  const { data: pendingData, error: pendingError } = await supabase
    .from('submissions')
    .select(
      'id, status, created_at, users:user_id(full_name, org_name), documentType:document_type_id(name), submission_versions!submission_id(version_number, activity_proposal_details(activity_title))',
    )
    .eq('status', 'submitted')
    .order('created_at', { ascending: false })
    .limit(10);
  ...
  if (pendingError) {
    console.error('Chairman pending submissions error:', pendingError.message);
    const { data: fallbackPending } = await supabase
      .from('submissions')
      .select('id, status, created_at, users:user_id(full_name, org_name), documentType:document_type_id(name)')
      .eq('status', 'submitted')
      .order('created_at', { ascending: false })
      .limit(10);
    pendingSubmissions = fallbackPending;
  } else {
  ...
  const pendingCount = allSubmissions
    ...
    : 0;
  const approvedCount = allSubmissions
    ? allSubmissions.filter((s) =>
        ['dean approved', 'completed', 'waiting for accomplishment report'].includes(
          String(s.status || '').toLowerCase(),
        ),
      ).length
    : 0;
  const returnedCount = allSubmissions
    ? allSubmissions.filter((s) => s.status === 'returned').length
    : 0;
  const completedCount = allSubmissions
    ? allSubmissions.filter((s) => s.status === 'completed').length
    : 0;
  ...
  const formattedPending = (pendingSubmissions || []).map((doc) => {
    ...
    return {
      ...
      title: docTitle,
      type: (doc.documentType as Record<string, unknown>)?.name || 'Unknown',
      orgName: (doc.users as Record<string, unknown>)?.org_name || 'N/A',
      submitter: (doc.users as Record<string, unknown>)?.full_name || 'Unknown',
