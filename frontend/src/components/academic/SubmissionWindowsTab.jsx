      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-xs overflow-hidden">
        <table className="w-full text-left text-sm">
          <thead className="bg-[#073c2d] text-white font-bold border-b border-[#073c2d] uppercase tracking-wider text-xs">
            <tr>
              <th className="px-3 sm:p-4 text-white">Document Type</th>
                ...
                <tr key={win.id} className="hover:bg-gray-50/70 transition">
                  <td className="px-3 sm:p-4 font-extrabold text-gray-900">
                    <div className="flex items-center gap-2.5">
                      ...
                      <div>
                        <div className="text-xs sm:text-sm">{win.title}</div>
                        {dt && <div className="text-[10px] font-semibold text-gray-400">{dt.name}</div>}
                        <div className="md:hidden text-[10px] text-gray-500 font-semibold mt-0.5">
                  ...
                  <td className="px-3 sm:p-4">
                    {status === 'Accepting now' ? (
                      <span className="bg-emerald-100 text-emerald-800 border border-emerald-300 px-2 sm:px-3 py-0.5 sm:py-1 rounded-full text-[10px] sm:text-xs font-extrabold inline-flex items-center gap-1">
                        <CheckCircle2 size={12} className="text-emerald-600 shrink-0" />
                        <span className="truncate">Accepting Now</span>
                      </span>
                    ) : status === 'Opens later' ? (
                      <span className="bg-amber-100 text-amber-800 border border-amber-300 px-2 sm:px-3 py-0.5 sm:py-1 rounded-full text-[10px] sm:text-xs font-bold">
                        Opens Later
                      </span>
                    ) : (
                      <span className="bg-gray-100 text-gray-600 border border-gray-200 px-2 sm:px-3 py-0.5 sm:py-1 rounded-full text-[10px] sm:text-xs font-bold">
                        Closed
                      </span>
                    )}
                  </td>
