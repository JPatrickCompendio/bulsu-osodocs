...
/**
 * Resolves the "Generated By:" label for reports based on the active operating user.
 * For Organization Accounts:
 *   - If an active member (e.g. Vice President) is operating: "Name (Position)"
 *   - If Org President is operating: "Name (Organization President)"
 * For Staff/Admin Accounts: "Name (Position)" or "Name"
 */
export const getReportGeneratorName = (user, activeMember) => {
  const role = String(user?.role || '').toLowerCase();
  const isOrgAccount = role === 'org-president' || role === 'org_president';

  if (isOrgAccount) {
    if (activeMember && activeMember.full_name) {
      if (activeMember.position) {
        const rawPos = String(activeMember.position).trim();
        const posFormatted = rawPos.startsWith('(') ? rawPos : `(${rawPos})`;
        return `${activeMember.full_name} ${posFormatted}`;
      }
      if (activeMember.is_president || activeMember.id === 'president') {
        return `${activeMember.full_name} (Organization President)`;
      }
      return activeMember.full_name;
    }
    if (user?.full_name) {
      const rawPos = user.position ? String(user.position).trim() : 'Organization President';
      const posFormatted = rawPos.startsWith('(') ? rawPos : `(${rawPos})`;
      return `${user.full_name} ${posFormatted}`;
    }
  ...
  if (user?.full_name) {
    if (user.position) {
      const rawPos = String(user.position).trim();
      const posFormatted = rawPos.startsWith('(') ? rawPos : `(${rawPos})`;
      return `${user.full_name} ${posFormatted}`;
    }
